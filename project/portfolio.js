...
(function () {
  ...
  if (loop) {
    ...
    setStep(0);
    function start() { if (timer || reduce) return; timer = setInterval(function () { setStep(cur + 1); }, 2400); }
    ...
    steps.forEach(function (s, idx) {
      s.addEventListener('mouseenter', function () { stop(); setStep(idx); });
      s.addEventListener('mouseleave', function () { start(); });
    });
  }
